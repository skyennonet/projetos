...
class ExcelSearchReplaceApp:
    ...
    def search_term_in_workbook(self, term_to_search, new_text):
        ...
        if term_found:
            self.search_result_text.insert(tk.END, f"Item '{term_to_search}' substituído por '{new_text}' nas planilhas.\n")
        else:
            self.search_result_text.insert(tk.END, f"Item '{term_to_search}' não encontrado nas planilhas.\n")
